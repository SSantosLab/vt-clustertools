...
def jack(i,zd, ra_rad, dec_rad, rand_ra, rand_dec, unique_pix, pix_gal, pix_rand):
    ...
    galaxy_catalog = treecorr.Catalog(ra = ra_jack[(i)], dec = dec_jack[(i)], ra_col='RA', dec_col='DEC', ra_units='rad', dec_units='rad')
...
if ((RA_LIM == True) or (DEC_LIM == True)):
    ...
    new_file = 'des40a/output_files/fits_files/data_cut.fits'
    data.write(new_file) #writes out fits file with new masked galaxy sample
else:
    data = full_data
    new_file = galaxy_sample
# create galaxy catalog
galaxy_catalog = treecorr.Catalog(new_file, ra_col=RA_COL, dec_col=DEC_COL, ra_units='deg', dec_units='deg')
...
for zd in range (0,25):
   ...
   boxID = str(zd) + 'redshift_bin#'
   mask = (z>(z_min[zd])) & (z<(z_max[zd]))
   file_name = 'des40a/output_files/fits_files/' + boxID + '.fits'  
   data[mask].write(file_name)
   galaxy_catalog = treecorr.Catalog(file_name, ra_col=RA_COL, dec_col=DEC_COL, ra_units='deg', dec_units='deg') 
   ra_rad = galaxy_catalog.ra
   dec_rad = galaxy_catalog.dec
...
if (GET_GRAPHS == True):
    ...
    fig.tight_layout()
    fig.savefig('des40a/output_files/plots/corrfunction_grid.png')
    plt.close(fig)
...
if (GET_GRAPHS == True):    
    ...
    fig.savefig('des40a/output_files/plots/correlation_matrices.png')
    plt.close(fig)
...
if (GET_GRAPHS == True):     
    ...
    ax2.set_ylabel('A')
    fig.savefig('des40a/output_files/plots/paramters_graphs.png')
    plt.close(fig)
