...
if (MAG_CUT==True):
    mag = numpy.array(full_data[MAG_COL])
    mag_min = config['mag_min']
    mag_max = config['mag_max']
    mag_mask = ((mag>mag_min)&(mag<mag_max))
    full_data = full_data[mag_mask]
    z= numpy.array(full_data[Z_COL])
    ra = numpy.array (full_data[RA_COL]) 
    dec = numpy.array(full_data[DEC_COL])
...
if (GET_GRAPHS == True):
    fig, ax = plt.subplots(5, 5,figsize = (20,20)) #linear plot of autocorrelation function fo each z bin
    rep = []
    for i in range(5):
        rep.append (5*i)
        for j in range(5):
    ...
    fig.tight_layout()
    fig.savefig(OUTPUTDIR+'/corrfunction_grid.png')
    plt.close(fig)
...
if (GET_GRAPHS == True):
    ...
    for i in range(5):
        ...
        for j in range(5):
            ...
            maxx = numpy.max(r_value[rep[i] + j])
            miny = numpy.min(xi_value[rep[i] + j])
            maxy = numpy.max(xi_value[rep[i] + j])
            ax[i,j].axis([(minx/1.5), (maxx*1.5), (miny/1.5), (maxy*1.5)])
            ax[i,j].loglog()
            for axis in [ax[i,j].xaxis, ax[i,j].yaxis]:
                ax[i,j].xaxis.set_major_formatter(StrMethodFormatter('{x:.2f}'))
                ax[i,j].xaxis.set_minor_formatter(NullFormatter())
                ax[i,j].yaxis.set_major_formatter(StrMethodFormatter('{x:.2f}'))
                ax[i,j].yaxis.set_minor_formatter(NullFormatter())
    fig.tight_layout()
    fig.savefig(OUTPUTDIR+'/log_corrfunction_grid.png')
    plt.close(fig)
    #plt.show()
...
if (GET_GRAPHS == True):     
    ...
    ax2.set_ylabel('A')
    fig.savefig(OUTPUTDIR+'/parameters_graphs.png')
    plt.close(fig)
